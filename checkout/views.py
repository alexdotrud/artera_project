...
def send_order_confirmation(order):
    ...
    subject = render_to_string(
        "checkout/email/confirmation_subject.txt", ctx).strip()
    text_body = render_to_string("checkout/email/confirmation_body.txt", ctx)
    try:
        html_body = render_to_string(
            "checkout/email/confirmation_body.html", ctx
        )
    except Exception:
        html_body = None
...
def checkout(request):
    """Process checkout: save order, create line items, init Stripe payment.
    ...
    if request.method == "POST":
        ...
        if order_form.is_valid():
                ...
                try:
                    ...
                    if isinstance(item_data, int):
                        ...
                        if isinstance(item_data, dict):
                            if (
                                "sizes" in item_data
                                and isinstance(item_data["sizes"], dict)
                            ):
                    ...
                    messages.error(
                        request,
                        (
                            "One of the artworks wasn't found in our database."
            ...
            if request.user.is_authenticated and "save_info" in request.POST:
                ...
                profile.address_line_delivery = (
                    cd.get("street_address1", "") or ""
                )
                profile.address_line_living = (
                    cd.get("street_address2", "") or ""
                )
...
def checkout_success(request, order_number):
    ...
    messages.success(
        request,
        f"Order successfully processed! A confirmation \
